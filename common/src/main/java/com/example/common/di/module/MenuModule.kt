...
//            Menu(Colors.SKY_BLUE, "底部导航控件", ARouterPath.ACTIVITY_BOTTOM_BAR),
        ...
        Menu(Colors.SKY_BLUE, "声波控件", ARouterPath.ACTIVITY_AUDIO_WAVE_VIEW),
...
//            Menu(Colors.SKY_BLUE, "翻书控件", ARouterPath.ACTIVITY_BOOK_PAGER),
        ...
        Menu(Colors.LEAF_GREEN, "ORM基础 - 更新数据", ARouterPath.ACTIVITY_ORM_UPDATE_DATA),
...
//            Menu(Colors.LEAF_GREEN, "数据缓存 - 缓存分页", ARouterPath.ACTIVITY_CACHE_PAGER),
        ...
        Menu(
            Colors.LEAF_GREEN,
            "复杂数据类型测试",
            ARouterPath.ACTIVITY_COMPLEX_DATA_TYPE_TEST
        ),
        Menu(Colors.LEAF_GREEN, "RESTful API响应测试", ARouterPath.ACTIVITY_NET_TEST),
    )

    @Provides
    fun provideMenuList(): MutableList<Menu> {
        return menus
    }

    @Provides
    fun provideAdapter(): MenuListAdapter {
        return MenuListAdapter(menus)
    }
}